...
``On Startup``メソッド（``UNIT.4dbase``コンポーネントがインストールされている場合は``00_TEST``）を実行してみてください。集計画面が表示されます。「集計 1」から「集計 3」は，それぞれ同じ内容の集計を実行しますが，メソッドの書き方が違います。「集計 1」は古典的な4Dコーディングです。「集計 3」は，ネットワークトラフィックを意識した書き方になっています。
...
<img width="540" alt="" src="https://user-images.githubusercontent.com/10509075/29163092-57b4aa50-7df6-11e7-83c4-aad32bfac943.png">
...
メソッドの実行時間はほとんど同じでも，バイト数およびリクエスト数にかなりの違いがあることがわかります。つまり，「集計 1」のコードは，ローカルモードでは良くても，クライアントサーバー版では，かなりパフォーマンスが低下する，ということがすでに予測できます。おなじことを達成するのに，約12倍のリクエスト数および約360倍のパケットサイズを要しているからです。
